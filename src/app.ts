...
const apollo = new ApolloServer({
  schema,
  formatError: (err) => {
    if (err.message.startsWith('Query engine exited with code 1') || err.message.includes('corrupted')) {
      // errorReporting.report(err.message);
      logger.error(`Fatal error, exiting: ${err.message}`);
      process.exit(1);
    }
    if (err.message.startsWith('prisma') || err?.extensions?.code === 'INTERNAL_SERVER_ERROR') {
